...
def fetch_close_prices(symbol, depth, aslist = True):
    ...
    close_prices = data['Close']
    close_prices = close_prices.to_frame().reset_index().rename(columns={"Close":symbol})
    return close_prices
...
def parse_asset_data(write, depth: float, assets: list):
    ...
    for ticker in assets:
        prices_df = fetch_close_prices(ticker, depth, aslist=False)
        if not data.empty:
            data = pd.merge(data, prices_df, on="Date")
        else:
            data = prices_df
